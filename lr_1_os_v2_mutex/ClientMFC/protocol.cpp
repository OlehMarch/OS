...
void Protocol::clientLogFileCreation() {
	logFile.Open(_T("ClientLogFile.log"), CStdioFile::modeCreate | CStdioFile::modeNoTruncate | CStdioFile::modeReadWrite | CStdioFile::typeText);
	logFile.SeekToEnd();
}
...
void Protocol::overallProtocol() {
	...
	getLogOfMutexWrite();
	getLogOfMapViewOfFile();
	getLogOfMutexReadOccupation();
	getLogOfMutexWriteOccupation();
	getLogOfMutexReadRelease();
	getLogOfMutexWriteRelease();
	setSeparator();
	logFileClose();
	ReleaseMutex(hMutex);
